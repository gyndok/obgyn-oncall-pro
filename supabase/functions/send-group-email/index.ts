...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!apiKey) return jsonResponse({ error: "Email is not configured (RESEND_API_KEY missing)." }, 500);
    ...
    const doctorIds: string[] = Array.isArray(payload?.doctorIds)
    ...
    if (!sendAll && doctorIds.length === 0) return jsonResponse({ error: "Choose at least one recipient." }, 400);

    const supabase = createClient(Deno.env.get("SUPABASE_URL")!, Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!);
    let q = supabase.from("doctors").select("id, name, email").eq("active", true);
    if (!sendAll) q = q.in("id", doctorIds);
    const { data: doctors, error } = await q.order("name");
    if (error) throw error;
    const skipped = sendAll ? 0 : doctorIds.length - (doctors?.length ?? 0);
    ...
    const { error: logError } = await supabase.from("group_email_log").insert({
      sent_by: user.id,
      subject,
      recipient_count: doctors?.length ?? 0,
      sent_count: sent.length,
      failed_count: failed.length,
    });
    if (logError) console.error("group_email_log insert failed", logError);

    return jsonResponse({ sent, failed, skipped });
  } catch (e) {
